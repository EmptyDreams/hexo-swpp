...
module.exports = (hexo, config, pluginConfig, swRules) => {
    ...
    /** 判断是否为 HTTP 缓存项 */
    const isHttpCache = url => {
        // noinspection JSUnresolvedReference
        const httpCaches = pluginConfig.json?.http
        if (!httpCaches) return false
        for (let reg of httpCaches) {
            if (url.match(new RegExp(reg))) return true
        }
        return false
    }
    ...
    const buildNewJson = path => new Promise(resolve => {
        ...
        eachAllFile(config.public_dir, path => {
            ...
            let content = null
            const isHttp = isHttpCache(url.href)
            if (findCache(url) || isHttp) {
                content = fs.readFileSync(path, 'utf-8')
                const key = decodeURIComponent(url.pathname)
                const md5 = crypto.createHash('md5').update(content).digest('hex')
                result[key] = isHttp ? `hc:${md5}` : md5
            }
            // 外链监控
            ...
            const handleLink = link => {
                ...
                if (url.hostname === domain || isExclude(url.href)) return
                const isHttp = isHttpCache(url.href)
                if (!(isHttp || findCache(url))) return
    ...
    const tidyDiff = (dif, expand) => {
        ...
        for (let it of dif) {
            // 当前文件的 URL
            const url = new URL(it.match(/^(https?|\/\/)/) ? it : nodePath.join(root, it))
            const cache = findCache(url)
            if (!cache) {
                if (!dif[it].startsWith('hc:')) {
                    logger.error(`[buildUpdate] 指定 URL(${url.pathname}) 未查询到缓存规则！`)
                    continue
                }
            } else if (!cache.clean) tidied.updateGlobal = true
